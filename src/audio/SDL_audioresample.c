...
static void LeastSquares(float *coeffs, float y0, float y1, float y2, float y3)
{
    // Least squares matrix for [0, 1/3, 2/3, 1]
    // [  1.0   0.0   0.0  0.0 ]
    // [ -5.5   9.0  -4.5  1.0 ]
    // [  9.0 -22.5  18.0 -4.5 ]
    // [ -4.5  13.5 -13.5  4.5 ]
    ...
    coeffs[0] = y0;
    coeffs[1] = -5.5f * y0 + 9.0f * y1 - 4.5f * y2 + y3;
    coeffs[2] = 9.0f * y0 - 22.5f * y1 + 18.0f * y2 - 4.5f * y3;
    coeffs[3] = -4.5f * y0 + 13.5f * y1 - 13.5f * y2 + 4.5f * y3;
}
...
static void GenerateResamplerFilter()
{
    ...
    for (i = 0; i < RESAMPLER_SAMPLES_PER_ZERO_CROSSING; ++i) {
        for (j = 0; j < RESAMPLER_ZERO_CROSSINGS; ++j) {
            ...
            // Calculate the least squares using values at [0, 1/3, 2/3, 1]
            LeastSquares(fwd, ys[0], ys[1], ys[2], ys[3]);
            LeastSquares(rev, ys[3], ys[2], ys[1], ys[0]);
        }
    }
}
