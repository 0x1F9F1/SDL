                                    ...
                                    const Uint8 *data, size_t len)
{
    ...
    size_t history_length = queue->history_length;

    if (len >= history_length) {
        SDL_memcpy(history_buffer, &data[len - history_length], history_length);
    } else {
        size_t preserve = history_length - len;
...
SDL_AudioTrack *SDL_CreateAudioTrack(
    ...
    void *userdata, SDL_ReleaseAudioBufferCallback callback)
{
...
static SDL_AudioTrack *CreateChunkedAudioTrack(SDL_AudioQueue *queue, const SDL_AudioSpec *spec)
{
    ...
    SDL_AudioTrack *track = SDL_CreateAudioTrack(queue, spec, chunk, 0, capacity, queue, FreeChunkedAudioBuffer);
...
int SDL_WriteToAudioQueue(SDL_AudioQueue *queue, const SDL_AudioSpec *spec, const Uint8 *data, size_t len)
{
    ...
    for (;;) {
        total += WriteToAudioTrack(track, &data[total], len - total);

        if (total == len) {
            break;
        }

        SDL_AudioTrack *next = CreateChunkedAudioTrack(queue, spec);
...
{
    SDL_AudioTrack *track = (SDL_AudioTrack *)*inout_iter;

    *inout_iter = track->next;
    SDL_copyp(out_spec, &track->spec);
    *out_flushed = track->flushed;

    return track->tail - track->head;
}

static const Uint8 *PeekIntoAudioQueuePast(const SDL_AudioQueue *queue, Uint8 *data, size_t len)
{
    const SDL_AudioTrack *track = queue->head;
    ...
    SDL_assert(past <= queue->history_length);
...
static const Uint8 *ReadFromAudioQueue(SDL_AudioQueue *queue, size_t len)
{
    SDL_AudioTrack *track = queue->head;

    const Uint8 *ptr = &track->data[track->head];

    SDL_assert(len <= track->tail - track->head);
    track->head += len;

    return ptr;
}

static const Uint8 *PeekIntoAudioQueueFuture(const SDL_AudioQueue *queue, Uint8 *data, size_t len)
{
    const SDL_AudioTrack *track = queue->head;
                                    ...
                                    Uint8 *scratch)
{
    ...
    if (src_present_bytes) {
        ConvertAudio(present_frames, ReadFromAudioQueue(queue, src_present_bytes), src_format, src_channels, dst, dst_format, dst_channels, scratch);
